...
class PdfDocumentWeb extends PdfDocument {
  ...
  Future<PdfPage> _getPage(PdfjsDocument document, int pageNumber) async {
    ...
    return PdfPageWeb._(document: this, pageNumber: pageNumber, page: page, width: vp1.width, height: vp1.height, rotation: PdfPageRotation.values[page.rotate ~/ 90]);
  }
  ...
  Future<PdfDest?> _getDestination(JSAny? dest) async {
    ...
    if (arr.length < 3) {
      ...
      params = List<double?>.unmodifiable(arr.sublist(2).map((v) => (v as JSNumber?)?.toDartDouble).cast<double?>());
    } else {
...
class PdfPageWeb extends PdfPage {
  ...
  Future<Uint8List> _renderRaw(
    ...
    final vp = page.getViewport(PdfjsViewportParams(scale: fullWidth / pageWidth, offsetX: -x.toDouble(), offsetY: -y.toDouble(), dontFlip: dontFlip));
  ...
  Future<List<PdfLink>> loadLinks({bool compact = false}) async {
    ...
    for (final annot in annots) {
      ...
      if (_isRunningWithWasm) {
        rect = annot.rect.toDart.map((v) => (v).toDartDouble).cast<double>().toList();
      } else {
